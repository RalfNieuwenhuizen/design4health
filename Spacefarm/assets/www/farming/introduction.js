...
farming.Introduction = function (game) {
    ...
    this.text = new lime.Label().setFontSize(16).setPosition(this.center.x, this.center.y-15).setMultiline(true);

    this.button = new farming.Button('').setColor(SETTINGS.color.button).setAction(this.buttonAction, this);
        //.setPosition(SETTINGS.position.close_button).setSize(30,30).setText('Start');;
}
...
farming.Introduction.prototype.intro0 = function(){
	...
	this.button.setPosition(SETTINGS.position.center_button)
    .setSize(SETTINGS.size.button).setText('Start');
