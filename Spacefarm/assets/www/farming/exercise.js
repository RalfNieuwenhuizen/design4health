...
farming.Exercise = function (callbackName, target, onExerciseSuccess, onExerciseCancel) {
    ...
    var exercise = this;
    var options = typeof EXERCISES[callbackName].options == 'undefined' ? { frequency: 100 } : EXERCISES[callbackName].options;
...
farming.Exercise.prototype.checkRepetition = function (exercise, acc, threshold) {
 first=  acc[0];
last=acc[acc.length-1];
    for (var i = 1; i <= numPrevSample; i++){
        if(acc.length == 1) continue;
        var before = acc.length < i ? first : acc[acc.length-i];
        var prev = acc[acc.length-2];
        if(last-before > threshold && prev-before > threshold && goingDown && lastRise + peakMinSamplesBetween2 < acc.length) {
                goingDown = false;
                lastRise = acc.length;
                exercise.addRepetition();
        }
        if(before-last > threshold && before-prev > threshold) {
            goingDown = true;
        }
...
console.log('Acceleration jjY: ' + acceleration.y );


acc.push(acceleration.y);
console.log('array length is:' +acc.length);

exercise.checkRepetition(exercise, acc, 6);
...
exercise.checkRepetition(exercise, acc, 5);
    // first= acc[0];
...
farming.Exercise.prototype.playEndSound = function() {
    if(this.target && this.target.game && this.target.game.player.settings.sound == true) {
        var endsound = new Media('file:///android_asset/www/ex_end.ogg');
        endsound.play();
    }
}
...
var EXERCISES = {
    ...
    dynamic_chest : {
        ...
        'Repeat 15 times, increasing speed as you do so.'],
        example_frames: 8,
        horizontal: false,
        repetitions: 15,
        type: 'chest',
    ...
    mason_twist : {
        ...
        'Twist over to the left side and bring your hands toward the floor. Avoid using momentum - control the motion. Repeat 10 times.'],
        example_frames: 8,
        horizontal: true,
    ...
    wall_flapping : {
        ...
        'Stay in this position and stretch your arms parallel to the wall. Carefully move them up and down in a controlled motion. Repeat 15 times.'],
        example_frames: 10,
        horizontal: false,
        repetitions: 15,
        type: 'back',
    ...
    wall_ears : {
        ...
        description : ['Stand straight with your back to the wall, touching with your heels.',
        'Tuck in your chin and touch the wall also with the back of your neck.',
        'Stay in this position and stretch your arms parallel to the wall. Bring your arms up and cover your ears. Repeat 15 times.'],
        example_frames: 10,
        horizontal: false,
        repetitions: 15,
        type: 'back',
    ...
    wall_arm_pulls : {
        ...
        description : ['Stand straight with your back to the wall, touching with your heels.',
        'Tuck in your chin and touch the wall also with the back of your neck.',
        'Put your right arm up, flex it and pull down, just as you would climb a rope ladder. Repeat 15 times.'],
        example_frames: 8,
        horizontal: false,
        repetitions: 15,
        type: 'back',
    ...
    squats : {
        ...
        'Quickly reverse the motion until you return to the starting position. Repeat until the timer stops.'],
        example_frames: 9,
        horizontal: false,
        duration: 60, // in seconds
        type: 'legs',
